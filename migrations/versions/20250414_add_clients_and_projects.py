...
def upgrade():
    ...
    default_client_id = conn.execute(
        text("SELECT id FROM client WHERE name = 'Default Client'")
    ).fetchone()[0]
    ...
    conn.execute(text(f"UPDATE invoice SET client_id = {default_client_id}"))
    conn.execute(text(f"UPDATE timesheet SET client_id = {default_client_id}"))
