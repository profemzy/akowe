...
def test_invoice_create(client, auth, test_user, app, sample_timesheet):
    ...
    with app.app_context():
        unbilled = Timesheet.query.filter_by(user_id=test_user.id, status="pending").all()
...
def test_invoice_edit(client, auth, sample_invoice, app):
    ...
    response = client.post(
        f"/invoice/edit/{sample_invoice.id}",
        data={
...
def test_invoice_mark_paid(client, auth, app):
    ...
    with app.app_context():
        ...
        entries = (
            Timesheet.query.filter_by(user_id=auth.test_user.id, status="pending").limit(2).all()
        )
    ...
    # Mark as paid
    response = client.post(
        f"/invoice/mark_paid/{invoice_id}",
        data={"payment_method": "bank_transfer", "payment_reference": "TRX123456789"},
        follow_redirects=True,
    )
...
def test_company_name_from_env(client, auth, app, test_user, sample_timesheet, monkeypatch):
    ...
    # Get IDs of unbilled timesheet entries
    with app.app_context():
        unbilled = Timesheet.query.filter_by(user_id=test_user.id, status="pending").limit(1).all()
        entry_ids = [str(entry.id) for entry in unbilled]
