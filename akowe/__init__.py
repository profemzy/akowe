...
def create_app(test_config=None):
    ...
    migrate.init_app(app, db,
                     directory=os.path.join(os.path.dirname(app.root_path),
                                            'migrations'))
    ...
    def check_authentication():
        ...
        for ep in public_endpoints:
            if request.endpoint == ep or (
                isinstance(request.endpoint, str)
                and request.endpoint.startswith(ep + '.')
            ):
                is_public = True
                break
        ...
        is_api_request = (
            request.endpoint and isinstance(request.endpoint, str)
