    ...
    if request.method == "POST":
        try:
            ...
            existing_project = Project.query.filter_by(
                name=name, client_id=client_id, user_id=current_user.id
            ).first()
...
def view(id):
    ...
    timesheet_entries = (
        Timesheet.query.filter_by(project_id=project.id).order_by(Timesheet.date.desc()).all()
    )
...
def api_list():
    ...
    if client_id:
        ...
        projects = (
            Project.query.filter_by(user_id=current_user.id, status="active")
            .order_by(Project.name)
            .all()
        )
