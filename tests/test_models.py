...
def test_user_get_full_name(app):
    ...
    with app.app_context():
        ...
        user1 = User(username="user1", first_name="First", last_name="Last")
...
def test_income_expense_relationship(app):
    ...
    with app.app_context():
        ...
        income = Income(
            date=date(2025, 3, 21), amount=Decimal("10000.00"), client="Client", project="Project"
        )
        ...
        total_income = app.db.session.query(func.sum(Income.amount)).scalar() or Decimal("0.00")

        total_expense = app.db.session.query(func.sum(Expense.amount)).scalar() or Decimal("0.00")
